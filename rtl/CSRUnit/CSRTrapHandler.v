module CSR_trap_handler(
	...
	/*
	always @(posedge mret or posedge ecall or posedge raiseInterrupt) begin
		if (raiseInterrupt)
			$display("\n[interrupt] Time: %0t ns, cause: 0x%h  PC:0x%h ", $time, interruptCause, PC_A_o);
		if (mret)
			$display("\n[mret] Time: %0t ns, PC:0x%h ", $time, PC_E_o);
		if (ecall)
			$display("\n[ecall] Time: %0t ns, PC:0x%h ", $time, PC_E_o);
	end
	*/
