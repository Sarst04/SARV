module control_unit (
	...
	always @(opcode, funct3, funct5, funct7, funct12) begin
		{immExtendSelect, ALUSrcAType, ALUSrcBType  , ALUOpcode, memWrite, memRead, regWrite, jump, branch, writeBackSrcSelect, system, pause	} = 
		{ 3'b000		, 1'b0		 , 1'b0		    , 8'b0	   , 1'b0	 , 1'b0	  , 1'b0	, 1'b0, 1'b0  , 2'b0			  , 1'b0  , 1'b0    } ;
        case (opcode)
            LOAD : begin
            ...
            OP : begin
                ...
                case (funct3)
                    ...
                    F3_SLL, F3_ROL, F3_BCLR, F3_BINV, F3_BSET : begin    
						case (funct7)
							F7_SLL		: ALUOpcode = SLL;
